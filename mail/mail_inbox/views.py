...
#         image_path = r"C:\Users\ankit\PycharmProjects\MailBox\CamScanner-In-Python\test_img.jpg"
...
def process_image(request):
    if request.method == 'GET':
        ...
        image_path = "input_image/input.jpeg"
        image = cv2.imread(image_path)  # Read in the image
        if image is None:
            raise FileNotFoundError(f"Image not found at path: {image_path}")
        # image = cv2.imread(r'C:\Users\ankit\PycharmProjects\MailBox\mail\input_image\test_img.jpg')  # read in the image
